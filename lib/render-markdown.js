...
const ADMONITION_TYPES = new Set([
  ...
  'ANSWER', 'DEFINITION', 'PREREQUISITES', 'SOURCES', 'FAQ'
]);
const ADMONITION_PATTERN = /^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION|ANSWER|DEFINITION|PREREQUISITES|SOURCES|FAQ)](?:\n|$)/;
const GALLERY_IMAGE_PATTERN = /^!\[([^\]\r\n]+)]\((media\/[a-z0-9]+(?:-[a-z0-9]+)*\.(?:jpe?g|png|webp))(?:\s+"([^"\r\n]+)")?\)$/;
...
function admonitions(markdown) {
  markdown.core.ruler.after('block', 'gala_admonitions', (state) => {
    for (let index = 0; index < state.tokens.length - 2; index += 1) {
      ...
      const match = inline.content.match(ADMONITION_PATTERN);
  ...
  markdown.block.ruler.before('paragraph', 'gala_embed', (state, startLine, _endLine, silent) => {
    ...
    if (resolved == null) {
      ...
      if (resolved.provider === 'youtube') {
        state.env.galaExternalVideos = (state.env.galaExternalVideos ?? 0) + 1;
        if (state.env.galaExternalVideos > 1) {
          throw new TypeError('A post may contain at most one external video');
        }
      }
...
function galleryMarkup(content, mode) {
  const lines = content.split(/\r?\n/).filter((line) => line.trim() !== '');
  if (lines.length < 2 || lines.length > 10) {
    throw new TypeError('Gallery requires 2 to 10 images');
  }
  const items = lines.map((line) => {
    const match = GALLERY_IMAGE_PATTERN.exec(line.trim());
    if (!match) {
      throw new TypeError(
        'Gallery entries must be local Markdown images with meaningful alt text'
      );
    }
    const alt = markdown.utils.escapeHtml(match[1].trim());
    if (alt === '') throw new TypeError('Gallery images require meaningful alt text');
    const source = markdown.utils.escapeHtml(match[2]);
    const caption = match[3] == null
      ? ''
      : `<figcaption>${markdown.utils.escapeHtml(match[3])}</figcaption>`;
    return `<figure class="gala-gallery__item"><img src="${source}" alt="${alt}" loading="lazy" decoding="async">${caption}</figure>`;
  }).join('');
  const track = `<div class="gala-gallery__track"${mode === 'carousel' ? ' tabindex="0"' : ''}>${items}</div>`;
  if (mode === 'grid') {
    return `<section class="gala-gallery gala-gallery--grid" aria-label="Image gallery">${track}</section>`;
  }
  return `<section class="gala-gallery gala-gallery--carousel" aria-label="Image gallery" data-gala-carousel><button type="button" class="gala-gallery__previous" data-gala-carousel-previous aria-label="Previous image">‹</button>${track}<button type="button" class="gala-gallery__next" data-gala-carousel-next aria-label="Next image">›</button><span class="gala-visually-hidden" data-gala-carousel-status aria-live="polite">Image 1 of ${lines.length}</span></section>`;
}
...
markdown.renderer.rules.fence = (tokens, index, options, environment, self) => {
  const info = tokens[index].info.trim().toLowerCase();
  const language = info.split(/\s+/, 1)[0];
  if (language === 'gallery') {
    if (info !== 'gallery' && info !== 'gallery carousel') {
      throw new TypeError('Gallery mode must be grid or carousel');
    }
    const placeholder = randomBytes(18).toString('hex');
    environment.galaGalleries ??= new Map();
    environment.galaGalleries.set(
      placeholder,
      galleryMarkup(tokens[index].content, info === 'gallery carousel' ? 'carousel' : 'grid')
    );
    return `<div class="gala-gallery-placeholder" data-gallery-token="${placeholder}"></div>`;
  }
...
const sanitizeOptions = {
  ...
  allowedAttributes: {
    ...
    aside: ['class', 'role'],
    code: ['class'],
    div: ['class', 'id', 'data-highlight-token', 'data-embed-token', 'data-gallery-token'],
    h1: ['id'],
    h2: ['id'],
    h3: ['id'],
    h4: ['id'],
  ...
  allowedClasses: {
    aside: [
      ...
      'admonition-warning', 'admonition-caution', 'admonition-answer',
      'admonition-definition', 'admonition-prerequisites', 'admonition-sources',
      'admonition-faq'
    ],
    div: ['gala-highlight-placeholder', 'gala-embed-placeholder', 'gala-gallery-placeholder'],
...
function render(source, inline, suppliedEnvironment) {
  ...
  environment.galaImages = new Map();
  environment.galaGalleries = new Map();
  environment.galaExternalVideos = 0;
  environment.galaWarnings = [];
  const unsafe = inline
  ...
  for (const [token, gallery] of environment.galaGalleries) {
    safe = safe.replace(
      `<div class="gala-gallery-placeholder" data-gallery-token="${token}"></div>`,
      gallery
    );
  }
