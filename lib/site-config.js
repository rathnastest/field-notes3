...
const AI_PUBLISHING_KEYS = Object.freeze([
  'indexing', 'aiSearch', 'modelTraining', 'reuse', 'commercialUse',
  'licenseUrl', 'confirmation', 'attestBuilds'
]);
const AI_PUBLISHING_DEFAULTS = Object.freeze({
  indexing: 'not-declared',
  aiSearch: 'not-declared',
  modelTraining: 'not-declared',
  reuse: 'not-declared',
  commercialUse: 'not-declared',
  licenseUrl: '',
  confirmation: '',
  attestBuilds: false
});
const AI_PUBLISHING_VALUES = Object.freeze({
...
const APPEARANCE_THEME_KEYS = Object.freeze([
  'id', 'version', 'repository', 'commitSha', 'cssSha256', 'cssBytes', 'baseManagedCssBytes'
]);
const APPEARANCE_THEME_CSS = 'static/assets/appearance-theme.css';
...
function optionalHttps(value, field) {
  if (value == null || value === '' || value === 'unavailable') return '';
  if (typeof value !== 'string' || [...value.trim()].length > 2048) {
    throw new TypeError(`${field} must be a string of at most 2048 characters`);
  }
  let parsed;
  try { parsed = new URL(value.trim()); } catch { throw new TypeError(`${field} must be an HTTPS URL`); }
  if (parsed.protocol !== 'https:' || parsed.username || parsed.password || parsed.hash) {
    throw new TypeError(`${field} must be a credential-free HTTPS URL without a fragment`);
  }
  return value.trim();
}

export function aiPublishingPolicyDigest(value) {
  const declaration = Object.fromEntries(
    Object.keys(AI_PUBLISHING_VALUES).map((key) => [key, value[key]])
  );
  declaration.licenseUrl = value.licenseUrl;
  return createHash('sha256').update(JSON.stringify(declaration)).digest('hex');
}

export function validateAiPublishing(value) {
  if (value == null) return AI_PUBLISHING_DEFAULTS;
  if (Array.isArray(value) || typeof value !== 'object') {
    throw new TypeError('aiPublishing must be a mapping');
  }
  const unknown = Object.keys(value).filter((key) => !AI_PUBLISHING_KEYS.includes(key));
  if (unknown.length > 0) {
    throw new TypeError(`Unsupported aiPublishing option: ${unknown.join(', ')}`);
  }
  const normalized = { ...AI_PUBLISHING_DEFAULTS };
  for (const [field, allowed] of Object.entries(AI_PUBLISHING_VALUES)) {
    const selected = value[field] ?? 'not-declared';
    if (!allowed.includes(selected)) {
      throw new TypeError(`Unsupported aiPublishing.${field}: ${selected}`);
    }
    normalized[field] = selected;
  }
  normalized.licenseUrl = optionalHttps(value.licenseUrl, 'aiPublishing.licenseUrl');
  if (normalized.commercialUse === 'license-required' && normalized.licenseUrl === '') {
    throw new TypeError('aiPublishing.licenseUrl is required when commercial use requires a license');
  }
  if (normalized.commercialUse !== 'license-required' && normalized.licenseUrl !== '') {
    throw new TypeError('aiPublishing.licenseUrl is only valid when commercial use requires a license');
  }
  if (value.attestBuilds != null && typeof value.attestBuilds !== 'boolean') {
    throw new TypeError('aiPublishing.attestBuilds must be a boolean');
  }
  normalized.attestBuilds = value.attestBuilds === true;
  const declared = Object.keys(AI_PUBLISHING_VALUES)
    .some((field) => normalized[field] !== 'not-declared');
  const incomplete = Object.keys(AI_PUBLISHING_VALUES)
  ...
  if (normalized.reuse === 'block'
    ...
    throw new TypeError('aiPublishing.reuse cannot block all reuse while an automated use is allowed');
  }
  ...
  const confirmation = value.confirmation == null || value.confirmation === 'unavailable'
    ? '' : value.confirmation;
  if (typeof confirmation !== 'string' || (confirmation !== '' && !/^[a-f0-9]{64}$/.test(confirmation))) {
    throw new TypeError('aiPublishing.confirmation must be unavailable or a lowercase SHA-256 digest');
  }
  const digest = aiPublishingPolicyDigest(normalized);
  if (declared && confirmation !== digest) {
    throw new TypeError(
      `AI publishing declarations are not confirmed. Set aiPublishing.confirmation to ${digest} after reviewing the exact policy.`
    );
  }
  if (!declared && confirmation !== '') {
    throw new TypeError('Remove aiPublishing.confirmation when no rights declaration is selected');
  }
  return Object.freeze({
    ...normalized,
    confirmation,
    policyDigest: digest,
    declared,
    rslEnabled: declared && confirmation === digest
  });
}
...
function appearanceThemeInteger(value, field) {
  if (!Number.isSafeInteger(value) || value < 1) {
    throw new TypeError(`appearanceTheme.${field} must be a positive integer`);
  }
  return value;
}

export async function validateAppearanceTheme(value, root = process.cwd()) {
  if (value == null) return null;
  if (Array.isArray(value) || typeof value !== 'object') {
    throw new TypeError('appearanceTheme must be a mapping');
  }
  const unknown = Object.keys(value).filter((key) => !APPEARANCE_THEME_KEYS.includes(key));
  if (unknown.length > 0 || APPEARANCE_THEME_KEYS.some((key) => value[key] == null)) {
    throw new TypeError('appearanceTheme fields are invalid');
  }
  if (typeof value.id !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value.id)
      || typeof value.version !== 'string'
      || !/^(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)(?:-[0-9A-Za-z.-]+)?$/.test(value.version)
      || typeof value.repository !== 'string'
      || !/^rathnasgala\/[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/.test(value.repository)
      || typeof value.commitSha !== 'string' || !/^[0-9a-f]{40}$/.test(value.commitSha)
      || typeof value.cssSha256 !== 'string' || !/^[0-9a-f]{64}$/.test(value.cssSha256)) {
    throw new TypeError('appearanceTheme identity is invalid');
  }
  const cssBytes = appearanceThemeInteger(value.cssBytes, 'cssBytes');
  const baseManagedCssBytes = appearanceThemeInteger(
    value.baseManagedCssBytes, 'baseManagedCssBytes');
  if (cssBytes > 32 * 1024 || baseManagedCssBytes > Number.MAX_SAFE_INTEGER - cssBytes) {
    throw new TypeError('appearanceTheme CSS budget is invalid');
  }
  const file = path.resolve(root, APPEARANCE_THEME_CSS);
  const relative = path.relative(path.resolve(root), file);
  if (relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new TypeError('appearanceTheme CSS escaped the checkout');
  }
  const metadata = await lstat(file);
  if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.size !== cssBytes) {
    throw new TypeError('appearanceTheme CSS artifact is missing or has the wrong size');
  }
  const css = await readFile(file);
  if (createHash('sha256').update(css).digest('hex') !== value.cssSha256) {
    throw new TypeError('appearanceTheme CSS artifact failed integrity verification');
  }
  return Object.freeze({
    id: value.id,
...
export async function loadSiteConfiguration({
  ...
  config.appearanceTheme = await validateAppearanceTheme(config.appearanceTheme, checkout);
  ...
  config.aiPublishing = validateAiPublishing(config.aiPublishing);
